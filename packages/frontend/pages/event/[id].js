...
function Event({ event }) {
  ...
  const newRSVP = async () => {
    try {
      ...
      setSuccess(false);
      setMessage(`Could not create RSVP, ERROR: ${error}`);
      setLoading(false);
      console.log(error);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <section className="relative py-12">
        ...
        <h1 className="text-3xl tracking-tight font-extrabold light:text-gray-900 dark:text-gray-700 sm:text-4xl md:text-5xl mb-6 lg:mb-12">
          {event.name}
        </h1>
            ...
            { event.eventTimestamp > currentTimestamp ? (
              account ? (
                checkIfRSVPed === true ? (
                  ...
                  onClick={newRSVP}
                  >
                    {ethers.utils.formatEther(event.deposit)} MATIC Required to RSVP
                  </button>
              ...
              <span className="w-full text-center px-6 py-3 text-base font-medium rounded-full border-2 border-gray-200">
                Event occured on { formatTimestamp(event.eventTimestamp) }
              </span>
            )}
            <div className="flex item-center">
