...
function PastEvent({ event }) {
  ...
  const confirmAttendee = async (attendee) => {
    try {
      ...
      if (rsvpContract) {
        ...
        setLoading(true);
        console.log("Minting (confirmAttendee)...", txn.hash);

        await txn.wait();
        console.log("Minted (confirmAttendee) --", txn.hash);
        setSuccess(true);
  ...
  const confirmAllAttendees = async () => {
    ...
    try {
      ...
      if (rsvpContract) {
        ...
        setLoading(true);
        console.log("Minting (confirmAllAttendees) ...");
        await txn.wait();
        console.log("Minted (confirmAllAttendees) --");

        setSuccess(true);
        setMessage("Attendance for all attendees has been confirmed");
        setLoading(false);
      } else {
        console.log("Ethereum object doesn't exist");
      }
  ...
  // Check if the passed in address has confirmed to the event
  function checkIfConfirmed(event, address) {
    for (let i=0; i < event.confirmAttendees.length; i++) {
      let confirmedAttendeeAddress = event.confirmAttendees[i].attendee.id;
      if (confirmAttendeeAddress.toLowerCase() == attendee.toLowerCase()) {
        return true;
      }
    }
    return false;
