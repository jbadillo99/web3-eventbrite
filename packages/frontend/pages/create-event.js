...
export default function CreateEvent() {
  ...
  async function handleSubmit(e) {
    ...
    // Organize state body to pass to API endpoint which will be stored on Web2.storage
    const body = {
      name: eventName,
  ...
  const createEvent = async (cid) => {
    try {
      ...
      setSuccess(false);
      setMessage(`Could not create your event, Error:${error.message}`);
      setLoading(false);
      console.log(error);
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <section className="relative py-12">
          ...
          {
            !success && (
              <h1 className="text-3xl tracking-tight font-extrabold light:text-gray-900 dark:text-gray-700 sm:text-4xl md:text-5xl mb-4">
